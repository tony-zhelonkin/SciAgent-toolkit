...
- Debugging column name mismatches between R compute and Python visualization layers
...
**Route elsewhere for:**
- Running GSEA itself (gene ranking, permutation testing) → `references/msigdb.md`
- Parsing raw gene set files (GMT, GMX, TSV) → `references/custom-db.md`
- Creating dotplots, heatmaps, or running sum plots → `references/visualization.md`
...
> **The `02_analysis/` paths below are a project convention.**
...
entry <- data.frame(
  pathway_id        = naming_result$pathway_id,   # "GATOM_GLUTAMINE_DEPENDENT_REDOX___ANAPLEROSIS"
  pathway_name      = naming_result$pathway_name, # "Glutamine-Dependent Redox & Anaplerosis"
  nes               = ifelse(is.na(avg_logfc), 0, avg_logfc),  # pseudo-NES from edge logFC
  pvalue            = geom_mean_pval,
  padj              = geom_mean_pval,              # no multiple testing for single module
...
- [ ] **All databases present:** `unique(df$database)` includes all expected databases
...
- **Symptom:** The master table grows each time an append script is re-run. Row counts increase without removing old ones.
- **Cause:** The `filter(database != "X")` step uses a string that does not exactly match the `database` values already in the CSV. For example, the initial creation used `"TransportDB"` but the append script filters on `"transportdb"`.
...
- **Symptom:** Downstream visualizations crash or produce extreme axis ranges.
- **Cause:** Some pathways have `padj = 0` (exact zero from fgsea when enrichment is extreme), producing `-log10(0) = Inf`.
- **Fix:** The normalization function caps `neg_log_padj` at 16: `ifelse(is.infinite(neg_log_padj), 16, neg_log_padj)`. Verify this cap is applied. If manually constructing rows, use `neg_log_padj = -log10(max(padj, 1e-16))`.

### Pitfall: Leading Edge Size Parsed as Zero

- **Symptom:** `leading_edge_size` is 0 for all rows, or `gene_ratio` is 0.
- **Cause:** The `core_enrichment` column contains comma-separated genes instead of slash-separated, or the column is NA/empty.
- **Fix:** Check the separator: `head(df$core_enrichment, 1)`. If commas are used, pre-convert: `df$core_enrichment <- gsub(",", "/", df$core_enrichment)`.

### Pitfall: padj Column Ambiguity (p.adjust vs qvalue)

- **Symptom:** Different databases show different p-value scales in the master table.
- **Cause:** clusterProfiler's `gseaResult@result` contains both `p.adjust` and `qvalue` columns. Some GSEA runs populate one, some populate the other.
- **Fix:** The project-local function (`02_analysis/helpers/normalize_gsea.R`) uses: `if "qvalue" in colnames -> use qvalue, else use p.adjust`. Be consistent within a project.
