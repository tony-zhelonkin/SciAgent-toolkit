...
> Stage 4 of the bulk-rnaseq-gsea pipeline. See `../SKILL.md` for the full pipeline map and routing decision tree.
...
**Route elsewhere for:**
- Running GSEA analysis itself → `references/msigdb.md`
- Building custom gene sets → `references/custom-db.md`
- Assembling `master_gsea_table.csv` → `references/master-tables.md`
...
|   |   +-- Separated up/down       -->  gsea_dotplot(filterBy="NES_positive"/"NES_negative")
|   |
...
> equivalents. `<toolkit>/` means wherever this project vendors
...
combined_df <- dplyr::bind_rows(all_pathways) %>%
  mutate(
...
master   <- dplyr::bind_rows(readr::read_csv("03_results/tables/master_gsea_table.csv"), new_rows)
...
- [ ] **NES color scale is symmetric:** Blue-white-orange centered at zero
...
- **Cause:** `enrichplot::gseaplot2()` internally uses `Description` for color mapping, but `gsea_running_sum_plot()` names palette entries by `ID`. For MSigDB databases, Description and ID match. For custom databases, they diverge.
...
- **Cause:** The master CSV table is missing required columns defined in `config/pipeline.yaml` under `schemas.master_gsea_table.required_columns`.
...
- **Cause:** The similarity computation uses different metrics for different entity-pair types. TF-Pathway uses Overlap coefficient; PROGENy-Pathway uses Jaccard.
...
- **RNAseq-toolkit:** `<toolkit>/scripts/GSEA/GSEA_plotting/`
- **Visualization workflow docs:** `<toolkit>/docs/GSEA-workflow/04-output-artifacts-and-visualization.md`
- **Core pipeline docs:** `<toolkit>/docs/GSEA-workflow/01-core-pipeline-and-toolkit.md`
