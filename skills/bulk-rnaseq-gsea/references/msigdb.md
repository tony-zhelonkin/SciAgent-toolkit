...
> Stage 1 of the bulk-rnaseq-gsea pipeline. See `../SKILL.md` for the full pipeline map and routing decision tree.
...
**Route elsewhere for:**
- Custom/external gene sets (MitoCarta, TransportDB, GMT files) → `references/custom-db.md`
- Plotting GSEA results (dotplots, barplots, heatmaps) → `references/visualization.md`
- Creating master CSV tables from gseaResult objects → `references/master-tables.md`
...
|  |
|  +- Single database run?          -> Quick Start (this reference)
|  +- Full multi-database pipeline? -> Progressive Depth: Intermediate
|  +- Need to visualize results?    -> references/visualization.md
|  +- Need master CSV export?       -> references/master-tables.md
|
...
# Inspect top results
head(gsea_hallmark@result[order(gsea_hallmark@result$p.adjust),
                           c("ID", "NES", "p.adjust", "setSize")])
```
...
|-----------|-------|-----------|
| `eps` | `0` | Enables exact p-value calculation via fgsea adaptive multi-level splitting. Without this, extremely significant pathways get truncated p-values (all at ~1e-4). |
| `nPermSimple` | `100000` | Permutation count for precision. 100K sufficient for most analyses. |
| `pvalueCutoff` | `1.0` | Retains ALL pathways. Filtering happens downstream at visualization/export, avoiding expensive re-runs at different thresholds. |
...
all_gsea <- load_or_compute(
  ...
  compute_fn      = function() c(gsea_h_c2, gsea_c5)
)
```

The final `all_gsea` named list:
...
if (use_new_api) {
  ...
  msigdb_df <- msigdbr(db_species = db_species, species = species,
...
The `01_modules/` rows are the vendored toolkit; the `02_analysis/` rows are the
transcribed project's layout, shown as one worked arrangement.
...
- [ ] **Non-empty results:** `nrow(result@result) > 0` for every database (zero rows = no gene sets were tested)
- [ ] **Checkpoint files exist:** `file.exists("03_results/checkpoints/1.1_gsea_H_C2.rds")` and `1.1_gsea_C5.rds`
- [ ] **All databases present:** `names(all_gsea)` contains all expected keys
- [ ] **Biological plausibility:** Hallmark results include known biology (e.g., E2F targets, MYC targets if proliferation is affected)
- [ ] **P-value precision:** Very significant pathways have p-values well below 1e-10 (confirms `eps=0`; if all p-values are truncated at ~1e-4, eps was not set to 0)
...
- **Symptom:** Running C2 without specifying subcollection returns all of C2 (thousands of gene sets) instead of just one subcollection.
- **Cause:** Subcollection was `""` or `NULL` when a specific subcollection was intended.
- **Fix:** Verify the config entry. For Hallmark (`H`), empty subcollection is correct. For C2, C3, C5, always specify the subcollection (e.g., `"CP:KEGG"`, `"GO:BP"`).
...
- **Fix:** Verify `config.R` is sourced before `load_or_compute()`. Check `file.exists(file.path(DIR_CHECKPOINTS, "1.1_gsea_H_C2.rds"))` returns TRUE.
