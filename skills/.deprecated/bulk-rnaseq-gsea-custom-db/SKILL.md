---
name: bulk-rnaseq-gsea-custom-db
description: "Custom database GSEA integration -- prepares non-MSigDB gene set databases (MitoCarta, TransportDB, mitoXplorer, GATOM, GMT/GMX/CSV) for clusterProfiler GSEA and normalizes results into the master table schema. Use when adding a new external gene set database to an existing GSEA pipeline, converting gene IDs across species or ID types, or building T2G/T2N data frames from raw reference files. For MSigDB collections (Hallmark, KEGG, Reactome, GO) use bulk-rnaseq-gsea-msigdb. For result export or visualization use bulk-rnaseq-gsea-master-tables or bulk-rnaseq-gsea-visualization."
license: MIT
metadata:
  skill-author: SciAgent-toolkit
  last-reviewed: 2026-04-14
  version: 1.0.0
  upstream-docs: https://yulab-smu.top/biomedical-knowledge-mining-book/universal-api.html
  category: analysis
  tier: standard
  tags:
    - gsea
  ...
  contraindications:
    - "Do not use for MSigDB collections (Hallmark, KEGG, Reactome, GO). Use bulk-rnaseq-gsea-msigdb instead."
    - "Do not use for result export or visualization. Use bulk-rnaseq-gsea-master-tables or bulk-rnaseq-gsea-visualization instead."
---
...
- MSigDB collections (Hallmark, C2, C3, C5, etc.) -- use `bulk-rnaseq-gsea-msigdb`
- Visualizing GSEA results (dotplots, heatmaps, running sum) -- use `bulk-rnaseq-gsea-visualization`
- Building or appending to master CSV tables -- use `bulk-rnaseq-gsea-master-tables`
...
|   |
|   +-- GMT/GMX file
|   |   +-- Human genes, mouse analysis? --> parse_gmx() + homologene conversion (Section: GMX)
|   |   +-- Same species?                --> parse_gmx() or parse_gmt() directly (Section: GMX)
|   |
|   +-- TSV/CSV with gene column + category column
|   |   +-- Gene symbols match your species?  --> Group by category, direct passthrough (Section: TSV)
|   |   +-- Gene IDs are RefSeq/Ensembl?      --> AnnotationDbi conversion first (Section: CSV)
|   |   +-- Gene IDs are other species?        --> homologene first, then group (Section: GMX)
|   |
...
# 7. Normalize to master table schema and append
# (see Progressive Depth: Advanced Usage)
```

**Verify it worked:**

```r
# T2G has exactly 2 columns with correct names
stopifnot(ncol(db$T2G) == 2)
stopifnot(all(colnames(db$T2G) == c("gs_name", "gene_symbol")))

# All gene set names carry the database prefix
stopifnot(all(grepl("^MYDB_", db$T2G$gs_name)))

# Gene sets are within size bounds
sizes <- table(db$T2G$gs_name)
stopifnot(all(sizes >= 5 & sizes <= 500))

# GSEA produced results
...
All results are normalized to the 13-column master table schema:

| Column | Type | Description |
|--------|------|-------------|
| `pathway_id` | character | Gene set ID with database prefix |
...
| `set_size` | integer | Genes in gene set |
| `leading_edge_size` | integer | Genes in leading edge |
| `gene_ratio` | numeric | leading_edge_size / set_size |
| `core_enrichment` | character | Leading edge genes, slash-separated |
| `database` | character | Source database name |
...
**Idempotent append pattern** -- safe to re-run without creating duplicates:
...
if (file.exists(master_file)) {
  ...
  master_df <- bind_rows(master_df, export_df)                   # Add fresh rows
...
**Convention:** `1.3.*.R` scripts prepare databases (parsing, ID conversion, filtering). `1.4.*.R` and later scripts execute GSEA and normalize results. All scripts after `1.5.create_master_tables.R` use the idempotent append pattern.
...
- **Symptom:** Master table row count doubles each time the script runs. Downstream volcano/dotplots show duplicate entries.
...
|---|---|---|
| MSigDB collections (Hallmark, KEGG, Reactome, GO) | `bulk-rnaseq-gsea-msigdb` | Alternative |
| Master table creation and export | `bulk-rnaseq-gsea-master-tables` | Next step |
| Dotplots, heatmaps, running sum plots | `bulk-rnaseq-gsea-visualization` | Next step |
