...
mkdir -p replacement/.sciagent
printf '{"version":2}\n' > replacement/.sciagent/manifest.json
changed=$("$SCIAGENT" link --project-dir "$TMPDIR_TEST/replacement" 2>&1) || {
...
[[ ! -e replacement/.sciagent/manifest.json ]] || {
    echo "FAIL [$_TEST_NAME] legacy manifest survived link" >&2
    exit 1
}

help=$("$SCIAGENT" link --help 2>&1) || {
