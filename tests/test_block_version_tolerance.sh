...
sed -i 's/SCIO:CRAFT v1 hash=/SCIAGENT:CRAFT v2 hash=/; s/END SCIO:CRAFT/END SCIAGENT:CRAFT/' AGENTS.md
cp AGENTS.md v2.orig

assert_eq "$(block_read AGENTS.md CRAFT)" "${body%$'\n'}" "legacy v2 body located"
assert_eq "$(block_stored_hash AGENTS.md CRAFT)" \
    "$(printf '%s' "$body" | scio_sha1_stream)" "v2 stored hash extracted"
...
cat > v1.expected <<'EOF'
<!-- BEGIN SCIO:CRAFT v1 hash=41c9105ed1d3fe88123bd360d947c909fb85fd07 -->
some body
<!-- END SCIO:CRAFT -->
EOF
