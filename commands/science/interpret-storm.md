...
Each designer consults `skills/figure-style/SKILL.md` (the dual print+screen variant contract, the legibility floors, the `save_overview()` figure+table+caption discipline) so the proposal is buildable by `/add-figure-variant` without redesign.
...
- **Figure-style variant plan** — the print + screen plan: panel layout, what `<stem>.print.pdf` (column geometry) and `<stem>.screen.png` (slide geometry) each show, the sub-layout (`_overview/` or `by_contrast/<c>/`), and (for an interactive viewer) the static fallback panel that still satisfies the figure-style contract.
...
5. **Every design is buildable by `/add-figure-variant` without redesign.** A proposal MUST declare a namespace token, a data contract, a print+screen variant plan (per the `figure-style` contract), and a claim tier (L0–L7). A design missing any of these cannot be handed off.
